...
from xeyal import anon, app, db, lang, queue, tg, yt
from xeyal.helpers import admin_check, buttons, can_manage_vc
    ...
    if action == "close":
        ...
        try:
            notice = await query.message.reply_text(
                query.lang["closed_by"].format(user), quote=False
            )
        except Exception:
            return
...
async def _help(_, query: types.CallbackQuery):
    ...
    if len(data) == 1:
        ...
        return await _edit(
            ...
            buttons.help_markup(query.lang, from_start=is_photo),
        )
    ...
    if data[1] == "back":
        ...
        return await _edit(text, buttons.start_key(query.lang, private))
    elif data[1] == "close":
        try:
            await query.message.delete()
    ...
    await _edit(
        ...
        buttons.help_markup(query.lang, True),
    )
